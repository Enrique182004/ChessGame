...
public class ChessGUI extends JFrame {
    ...
    private class ChessBoardPanel extends JPanel {
        ...
        // method for loading piece images into a set
        private void loadPieceImages() {
            ...
            for (String color : colors) {
                for (String type : types) {
                    String path = "/images/" + color + "_" + type + ".png";  // path must start with /
                    try {
                        ...
                        pieceImages.put(color + "_" + type, icon.getImage());
                    } catch (Exception e) {
        ...
        private void drawPiece(Graphics g, ChessPiece type, String color, char x, int y) {
            ...
            int screenY = (8 - y) * CELL_SIZE;
        
            String key = color.toLowerCase() + "_" + type.toString().toLowerCase();
            Image image = pieceImages.get(key);
        
            if (image != null) {
                g.drawImage(image, screenX, screenY, CELL_SIZE, CELL_SIZE, this);
            } else {
                // fallback if image is missing
                g.setColor(Color.RED);
                g.drawString("?", screenX + CELL_SIZE / 2, screenY + CELL_SIZE / 2);
            }
        }
    }
